 ...
 * Powers the live environment for SafeSight & LAISA Dashboards
 *
 * MODIFIED 2026-05-19:
 * - Added `useRealSocialData` flag. When the ComposioSocialBridge is active,
 *   simulated social engagement increments are disabled so real metrics
 *   from Instagram / GA / WhatsApp are displayed instead.
 */
...
const AgentOrchestrator = {
  ...
  // When ComposioSocialBridge is active, skip simulated social updates
  useRealSocialData: false,
  ...
  setupEventListeners() {
    ...
    // Listen for real social data from Composio bridge
    window.addEventListener('composio:socialMetrics', (e) => {
      this.useRealSocialData = true;
      if (e.detail.instagram?.instagramFollowers != null) {
        this.stats.socialEngagement = e.detail.instagram.instagramFollowers;
      }
    });
  },
  ...
  updateStats() {
    setInterval(() => {
      ...
      // Only simulate social engagement if real data is not available
      if (!this.useRealSocialData) {
